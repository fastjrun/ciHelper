...
public class OrderByParser {
    ...
    public static String converToOrderBySql(String sql, String orderBy) {
        ...
        try {
            ...
            String defaultOrderBy = PlainSelect
                    .orderByToString(orderByElements);
            if (defaultOrderBy.indexOf('?') != -1) {
                throw new RuntimeException("原SQL[" + sql
                        + "]中的order by包含参数，因此不能使用OrderBy插件进行修改!");
            }
            // 新的sql
            sql = select.toString();
        } catch (Throwable e) {
        ...
        if (selectBody instanceof PlainSelect) {
            ...
            if (operationList.getPlainSelects() != null
                ...
                List<PlainSelect> plainSelects = operationList
                        .getPlainSelects();
                List<OrderByElement> orderByElements = plainSelects.get(
                        plainSelects.size() - 1).getOrderByElements();
                plainSelects.get(plainSelects.size() - 1).setOrderByElements(
                        null);
                return orderByElements;
            }
        }
