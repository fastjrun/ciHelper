package com.fastjrun.orderbyhelper;

import com.fastjrun.mybatis.ConditionHelper;
import com.fastjrun.orderbyhelper.sqlsource.*;
...
public class OrderByHelper implements Interceptor {
    ...
    public static void processIntercept(Invocation invocation) throws Throwable {
        ...
        if (!hasOrderBy(ms)) {
            ...
            if (sqlSource instanceof StaticSqlSource) {
                msObject.setValue("sqlSource", new OrderByStaticSqlSource(
                        (StaticSqlSource) sqlSource));
            } else if (sqlSource instanceof RawSqlSource) {
                msObject.setValue("sqlSource", new OrderByRawSqlSource(
                        (RawSqlSource) sqlSource));
            } else if (sqlSource instanceof ProviderSqlSource) {
                msObject.setValue("sqlSource", new OrderByProviderSqlSource(
                        (ProviderSqlSource) sqlSource));
            } else if (sqlSource instanceof DynamicSqlSource) {
                msObject.setValue("sqlSource", new OrderByDynamicSqlSource(
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        try {
            if (getOrderBy() != null || ConditionHelper.hasCondition()) {
