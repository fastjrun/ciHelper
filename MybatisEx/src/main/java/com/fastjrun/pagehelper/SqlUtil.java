package com.fastjrun.pagehelper;

import com.fastjrun.mybatis.ConditionHelper;
import com.fastjrun.orderbyhelper.OrderByHelper;
import com.fastjrun.orderbyhelper.sqlsource.OrderBySqlSource;
import com.fastjrun.pagehelper.parser.Parser;
import com.fastjrun.pagehelper.parser.impl.AbstractParser;
import com.fastjrun.pagehelper.sqlsource.*;
...
public class SqlUtil implements Constant {
    ...
    public static Page getPageFromObject(Object params) {
        ...
        if (hasRequest && requestClass.isAssignableFrom(params.getClass())) {
            try {
                paramsObject = SystemMetaObject.forObject(getParameterMap
                        .invoke(params, new Object[] {}));
            } catch (Exception e) {
                // 忽略
            }
        } else {
    ...
    public static boolean isPageSqlSource(MappedStatement ms) {
        ...
        if (sqlSource instanceof PageSqlSource) {

            if (sqlSource instanceof PageStaticSqlSource) {
                msObject.setValue("sqlSource",
                        ((PageStaticSqlSource) sqlSource).getOriginal());
            } else if (sqlSource instanceof PageRawSqlSource) {
                msObject.setValue("sqlSource",
                        ((PageRawSqlSource) sqlSource).getOriginal());
            } else if (sqlSource instanceof PageProviderSqlSource) {
                msObject.setValue("sqlSource",
                        ((PageProviderSqlSource) sqlSource).getOriginal());
            } else if (sqlSource instanceof PageDynamicSqlSource) {
                msObject.setValue("sqlSource",
                        ((PageDynamicSqlSource) sqlSource).getOriginal());
            }
     ...
     * @throws Throwable
     */
    public static void processMappedStatement(MappedStatement ms, Parser parser)
            throws Throwable {
        SqlSource sqlSource = ms.getSqlSource();
        MetaObject msObject = SystemMetaObject.forObject(ms);
        ...
        if (tempSqlSource instanceof StaticSqlSource) {
            pageSqlSource = new PageStaticSqlSource(
                    (StaticSqlSource) tempSqlSource, parser);
            ...
            pageSqlSource = new PageProviderSqlSource(
                    (ProviderSqlSource) tempSqlSource, parser);
        } else if (tempSqlSource instanceof DynamicSqlSource) {
            pageSqlSource = new PageDynamicSqlSource(
                    (DynamicSqlSource) tempSqlSource, parser);
        } else {
        ...
        if (page == null) {
            if (params instanceof RowBounds) {
                ...
                if (offsetAsPageNum) {
                    ...
                    page = new Page(new int[] { rowBounds.getOffset(),
                            rowBounds.getLimit() }, rowBoundsWithCount);
